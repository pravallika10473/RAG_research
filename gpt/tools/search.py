...
class ContextualVectorDB:
    def __init__(self, name: str, voyage_api_key=None, openai_api_key=None):
    ...
    def situate_image_context(self, image_path: str, title: str) -> tuple[str, Any]:
        ...
        try:
            ...
            print(f"Error reading documents.json: {str(e)}")
            return "Error: Could not read document metadata", None

        # Get the title using fullcontext
        try:
            title = fullcontext("What is the title of the document?", pdf_path)
            if title.startswith("Error:"):
                print(f"Error getting title: {title}")
...
2. Detailed Technical Description of image:  
   ...
   - Tailor the description based on the image type:
     ...
     - Schematic: Describe the schematic of the circuit.
...
- Start with this text is from the document title  Example: "This text is from the document {title}"
                ...
                messages=[
                    {
                        ...
                        "content": [
                            {
                                "type": "text",
                                "text": IMAGE_CONTEXT_PROMPT.format(title=title),
                            },
                            {
                                "type": "image_url",
                                "image_url": {
            ...
            for item in items_to_process:
                try:
                    if 'content' in item[1]:  # It's a chunk
                        ...
                        #     self.token_counts['cache_read'] += usage.cache_read_tokens
                        #     self.token_counts['cache_creation'] += usage.cache_creation_tokens
                        ...
                        title = fullcontext("What is the title of the document?, just give me the title nothing else, no other text", doc["pdf_path"])
                        contextualized_text, usage = self.situate_image_context(image["path"], title)
                        # with self.token_lock:
                        #     self.token_counts['input'] += usage.total_tokens
                        #     self.token_counts['cache_read'] += usage.cache_read_tokens
                        #     self.token_counts['cache_creation'] += usage.cache_creation_tokens
...
def retrieve_advanced(query: str, db: ContextualVectorDB, es_bm25: ElasticSearchBM25, k: int, semantic_weight: float = 0.8, bm25_weight: float = 0.2):
    ...
    sorted_ids = sorted(
        ...
        key=lambda x: str(x)  # Convert to string for comparison
    )
...
def main(query: str, load_data: bool = False) -> dict:
    ...
    if load_data:
        ...
        for item in dataset:
            title = fullcontext("What is the title of the document?, just give me the title nothing else, no other text", item["pdf_path"])
            item["title"] = title
